...
- 1 to 4 single-file HTML interactive tools per module (vanilla JS, `localStorage`, Copy-as-markdown). The capstone tool exports BOTH a visual `pitch.html` AND `README.md`.
...
Pages typically rebuild in 30 to 90 seconds. Confirm with `status: built`.
...
- **Banned phrases:** "your group", "as a team", "appoint a notetaker", "round-robin", "pair up", "with a partner", "peer red-team", "breakout", "report back to the room".
- **Banned characters:** the em-dash (`—`) and en-dash (`–`) — never use them anywhere (slides, notes, glossaries, lab guides, briefs, tool strings, generators). Replace by meaning: label→definition `:` · clause break `,` · range `to` · compound/spectrum `-` · "no value" cell `·`. The plain hyphen `-` is fine. Generators must not emit them; `scripts/dedash.py` sweeps an existing tree region-aware (prose vs. code). See `voice.md` → "Banned characters."
...
- **The hands-on lab is the protected block**: the single largest segment (~36 to 40 min; keep the solo-build sub-row generous). Absorb cuts from lectures, Q&A, activities, and the close, **never the lab**.
...
13. **Arrows must NEVER cross other tiles, boxes, or other arrows.** They take the SHORTEST path to the target. All arrows in a diagram have the same `stroke-width`. Use cubic Béziers with matched tangents at every L→C and C→L join (C1-continuous). `stroke-linejoin="round"` and `stroke-linecap="round"` on every path. (See `visual-primitives.md` for the full list of arrow rules, getting this wrong took 4 to 5 iterations on multiple slides.)
...
21. **Toolkit slides age fast.** Audit the "PM's AI Toolkit" / "AI Stack" slide every regeneration: drop defunct products (e.g. Bing), swap in the most-hyped recent entrants. Keep count steady (8 to 12).
...
- [`scripts/add_lab_builder.py`](scripts/add_lab_builder.py), inject the **schema-driven in-guide deliverable builder** into a Lab Guide so you don't hand-code the JS per module. Describe the module's deliverable as a small JSON schema (field types: `select` / `text` / `textarea` / `group` of sub-fields / repeatable `rows` with text/select columns, plus an optional `subject` and `debrief`); the script injects the shared CSS + workspace + a vanilla-JS engine that renders the form, autosaves to `localStorage`, builds the deliverable markdown, and wires **Copy markdown** / **Download {filename}.md** + live preview. `python3 scripts/add_lab_builder.py --html "Modules/Module 2 - Lab Guide.html" --config m2.json`. Idempotent (re-run replaces the prior block); emits no em/en-dash. The markdown is auto-generated (each section is a `## label`; group to bullet list; rows to a markdown table). See `component-templates.md` → "Schema-driven lab builder (reusable engine)."
