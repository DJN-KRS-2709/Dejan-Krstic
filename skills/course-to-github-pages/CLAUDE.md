...
- **Final Project Brief is HTML at the course root** (`Final Project Brief.html`, linked `../Final Project Brief.html`), not a non-clickable root `.md`. **For the ten PS certifications the brief is generated, not written.** Design lives in `ps-content-library`'s `scripts/fpb/template.html` (byte-identical across all ten), content in `scripts/fpb/data/<shortCode>.json` (semantic fields, no HTML). Run `python scripts/build_final_project_briefs.py` to render, and `--check` must print `ALL 10 MATCH` before shipping: hand-editing a brief breaks that drift guard. Eight sections in order: hero, `scenario` (Context, Challenge, Goal, grounded in the course's own source), `build`, `repo` (create from template, not fork; setup video at that step plus a "view the source template" link; repos private by default), `tools` ("What you'll need"), `submit` (submit both, repo and presentation, on the Learning Platform), `classtime`, `faqs` (uniform rubric, universal FAQs identical across courses, course-specific ones on top). No dated years, "paid plan" not prices, official course names, no outbound navigation.
- **The Lab Guide is HTML and embeds the deliverable builder, for every module (not just M1).** `Module {N} - Lab Guide.html` carries the steps *and* an in-guide workspace that exports the deliverable via Copy markdown / Download .md (with `localStorage` autosave + live preview). Don't ship a guide that says *what* to do without giving learners *where* to do it, and don't ship the workspace for M1 only. Author a JSON schema (`select`/`text`/`textarea`/`group`/`rows`) and inject the shared engine with `python3 scripts/add_lab_builder.py --html "Modules/Module {N} - Lab Guide.html" --config mN.json`, don't re-implement the JS per module. See `component-templates.md` → "Schema-driven lab builder (reusable engine)."
