...
- **Resources & Templates = standardized, all-clickable cards.** Two groups (This module / Whole course) of `res-card` `<a href>`s — Notes HTML, Lab Guide HTML, Frameworks Card md, Glossary md, Template repo, Final Project Brief HTML, cumulative Frameworks + Glossary, (capstone) Prompt Generator. No decorative tiles, no dead hrefs — every card resolves to a file that exists.
