...
- `Curriculum Map.html`
- `Final Project Brief.html` (only outside `ps-content-library`; for the ten PS certifications it is generated, see below)
- `Tools Overview.html`
...
**The Final Project Brief is HTML at the course root** (`Final Project Brief.html`, linked `../Final Project Brief.html`), not a non-clickable root `.md`. **For the ten PS certifications the brief is generated, not written.** Design lives in `ps-content-library`'s `scripts/fpb/template.html` (byte-identical across all ten), content in `scripts/fpb/data/<shortCode>.json` (semantic fields, no HTML). Run `python scripts/build_final_project_briefs.py` to render, and `--check` must print `ALL 10 MATCH` before shipping: hand-editing a brief breaks that drift guard. Eight sections in order: hero, `scenario` (Context, Challenge, Goal, grounded in the course's own source), `build`, `repo` (create from template, not fork; setup video at that step plus a "view the source template" link; repos private by default), `tools` ("What you'll need"), `submit` (submit both, repo and presentation, on the Learning Platform), `classtime`, `faqs` (uniform rubric, universal FAQs identical across courses, course-specific ones on top). No dated years, "paid plan" not prices, official course names, no outbound navigation.

**For the ten PS certifications the brief is generated, not written.** Design lives in `ps-content-library`'s `scripts/fpb/template.html` (byte-identical across all ten), content in `scripts/fpb/data/<shortCode>.json` (semantic fields, no HTML). Run `python scripts/build_final_project_briefs.py` to render, and `--check` must print `ALL 10 MATCH` before shipping: hand-editing a brief breaks that drift guard. Eight sections in order: hero, `scenario` (Context, Challenge, Goal, grounded in the course's own source), `build`, `repo` (create from template, not fork; setup video at that step plus a "view the source template" link; repos private by default), `tools` ("What you'll need"), `submit` (submit both, repo and presentation, on the Learning Platform), `classtime`, `faqs` (uniform rubric, universal FAQs identical across courses, course-specific ones on top). No dated years, "paid plan" not prices, official course names, no outbound navigation.
...
30. **The Lab Guide is HTML and embeds the deliverable builder, for every module.** `Module {N} - Lab Guide.html` carries the steps *and* an in-guide workspace that exports the deliverable via Copy markdown / Download .md (with `localStorage` autosave + live preview). The lab slide's "Open Lab Guide ↗" links to it. Don't ship a guide that says *what* without giving learners *where*, and don't ship it for M1 only (Shipping AI Agents shipped static step guides for M2..M6 while only M1 had the workspace). Don't hand-code the JS per module: author a JSON schema (`select`/`text`/`textarea`/`group`/`rows`) and run `python3 scripts/add_lab_builder.py --html "Modules/Module {N} - Lab Guide.html" --config mN.json`. See `component-templates.md` → "Lab Guide with embedded deliverable builder" + "Schema-driven lab builder (reusable engine)."
...
- `scripts/add_lab_builder.py`, inject the schema-driven in-guide deliverable builder into a Lab Guide. `python3 scripts/add_lab_builder.py --html "Modules/Module {N} - Lab Guide.html" --config mN.json`. Schema field types: `select`/`text`/`textarea`/`group`/`rows` (+ optional `subject`, `debrief`); auto-generates the deliverable markdown, Copy/Download/`localStorage`/live-preview, idempotent, dash-clean.
- `scripts/populate_notes.py`, fill empty/stub Notes (Shareable) HTML chapter bodies from the matching `.md`.
- `scripts/md_to_reference_html.py`, render Frameworks/Glossary `.md` into styled HTML companions for the Resources cards.
