...
- **Standardized clickable Resources & Templates.** Two groups (This module / Whole course) of `res-card` links: Notes HTML, Lab Guide HTML, Frameworks Card **HTML**, Glossary **HTML**, Template repo, Final Project Brief HTML. No decorative tiles, no dead hrefs. New `resources_and_templates()` component + gotcha #30.
- **Frameworks + Glossary link HTML, never `.md`; cumulative pair is capstone-only.** The first build linked raw `.md` (renders as plain text on Pages) and put the cumulative "all 6" Frameworks + Glossary on *every* module. Fixed: render styled HTML companions with `scripts/md_to_reference_html.py` (per-module + cumulative, family document style); M1→second-to-last link only **their own** module's Frameworks + Glossary, and the cumulative "all 6" pair (+ Prompt Generator) appears **only on the final module**. New `scripts/md_to_reference_html.py` + tightened gotcha #30.
